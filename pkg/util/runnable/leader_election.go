...
// LeaderElection wraps the given runnable to implement
// manager.LeaderElectionRunnable with the desired setting.
...
// RequireLeaderElection wraps the given runnable, marking it as requiring
// leader election.
func RequireLeaderElection(runnable manager.Runnable) manager.Runnable {
	return LeaderElection(runnable, true)
}

// NoLeaderElection wraps the given runnable, marking it as not requiring
// leader election. Typical use: servers (gRPC / health) that every replica
// must run regardless of leadership.
